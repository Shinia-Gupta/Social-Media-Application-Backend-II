...
import mailSent from "../../middlewares/sendMail.middleware.js";
...
export class UserRepository {
  async register(userData) {
    try {
      ...
      if (savedUser) return { success: true, res: savedUser };
    } catch (error) {
  ...
  async getById(userId) {
    try {
      ...
      if (user) {
        ...
        return {
          success: false,
          error: { statusCode: 404, message: "This user does not exist!" },
        };
      }
    } catch (error) {
  ...
  async getAll() {
    ...
    if (users.length != 0) {
      ...
      return {
        success: false,
        error: { statusCode: 404, message: "Be the first user to post!" },
      };
    }
  }
  ...
  async updateDetails(userData, userId) {
    ...
    try {
      ...
      if (updatedUser) {
        ...
        return {
          success: false,
          error: { statusCode: 400, message: "User information invalid" },
        };
      }
    } catch (error) {
      return { success: false, error: { statusCode: 404, message: error } };
  ...
  async logoutAll(userId) {
    try {
      ...
      return {
        success: false,
        error: { statusCode: 500, message: "could not logout of all devices " },
      };
      // return {success:false, error:{statusCode:404,message:error}}
    }
  }

  async sendResetOtp(mailId) {
    try {
      const info = await mailSent(mailId);
      console.log("info by mailSent-", info);
      return { success: true, res: info };
    } catch (error) {
      console.log(error);
      return { success: false, error: { statusCode: 404, message: error } };
    }
  ...
  async verifyOtp(otpByUser,actualotp) {
    if (otpByUser == actualotp) {
      return { success: true, res: "otp verified" };
    } else {
      return {
        success: false,
        res: "Otp did not match! Please enter a valid otp",
        }
      };
    }
...
try {
    ...
    const user=await userModel.findOneAndUpdate({_id:new ObjectId(userId)},{$set:{password:newPass}},{returnOriginal: false});
    await user.save();
    console.log(user);
    return { success: true, res: user };
} catch (error) {
